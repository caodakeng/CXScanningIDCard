...
#import "CXCardInfoVC.h"

@interface CXScanningCardVC ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureMetadataOutputObjectsDelegate>
//身份证数据模型
@property (nonatomic,strong)    CXCardModel                 *model;
// 摄像头设备
...
@implementation CXScanningCardVC
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *torchItem = [[UIBarButtonItem alloc] initWithCustomView:_rightBtn];
    ...
    //初始化预览图层   保持纵横比+填充层边界 这一层zw直接向我们展示录制的画面
    ...
    //添加自定义s的扫描界面
    CXIDCardScaningView *scaningView = [[CXIDCardScaningView alloc] initWithFrame:self.view.frame];
    _faceDetectionFrame = scaningView.facePathRect;
...
//检测人脸是为了获得"人脸区域"，做"人脸区域"与"身份证人相框"的区域对比
//当前者在后者范围内的时候，蔡恩感截取到完整的身份证图像
-(void)captureOutput:(AVCaptureOutput *)output didOutputMetadataObjects:(NSArray<__kindof AVMetadataObject *> *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count>0) {
        NSLog(@"1");
        AVMetadataMachineReadableCodeObject *metadataObject = metadataObjects.firstObject;
        
        //只有当人脸区域在小框内时，才去捕获此时的这一帧图像
...
-(void)captureOutput:(AVCaptureOutput *)output didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection{
    ...
    if ([_outPutSetting isEqualToNumber:[NSNumber numberWithInt:kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange]] || [_outPutSetting isEqualToNumber:[NSNumber numberWithInt:kCVPixelFormatType_420YpCbCr8BiPlanarFullRange]]) {
        ...
        if ([output isEqual:_videoDataOutput]) {
        ...
        if (ret <= 0) {
            ...
            if (_model){
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    CXCardInfoVC *vc = [CXCardInfoVC new];
                    vc.model = self.model;
